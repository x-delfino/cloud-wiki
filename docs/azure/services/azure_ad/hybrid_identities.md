There are a couple of ways in which users, groups and computers can exist in Azure AD (AAD):
- **Cloud Identity**: These accounts only exist in AAD, they have been created in cloud and are managed here.
- **Hybrid Identity**: These accounts have been synchronised from an on-premise Active Directory and are primarily managed there. This is what we'll be discussing here.
...
There are a fair few different service accounts involved for Hybrid Identities, when reviewing these, it's a good idea to check their permissions to ensure they have not been overly assigned.
